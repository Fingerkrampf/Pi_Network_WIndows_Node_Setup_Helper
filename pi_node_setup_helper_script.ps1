...
⚠️ Nutzung auf eigene Gefahr – getestet unter Windows 10/11 Pro mit aktuellen Patches
--------------------------------------------------------------------------------------
#>
...
return
    ...
    if ($job.State -ne 'Completed') {
        Write-Host "`nWARNUNG: Komponenten wurden nach $maxWait Sekunden nicht vollständig aktiviert." -ForegroundColor Red
        Write-Log "WSL2-Komponenten wurden nach Timeout nicht aktiviert." "WARN"
        ...
        if ($restartNow -match '^[Jj]$') {
            try {
                ...
                $loadedPath = Get-Content $pathFile

		$Action = New-ScheduledTaskAction -Execute "powershell.exe" -Argument "-NoProfile -WindowStyle Hidden -ExecutionPolicy Bypass -File `"$scriptPath`" -resumeWSL"
            ...
            Write-Host "Bitte führen Sie nachträglich einen Neustart durch, um WSL2 zu aktivieren." -ForegroundColor Yellow
        }
    } else {
        Remove-Job $job | Out-Null
    }
...
function Ensure-HostKeyAccepted {
    ...
    $process = Start-Process -FilePath $plink `
        -ArgumentList "-pw", $password, "$user@$serverIp", "exit" `
